...
| --- | --- | --- |
| Establish current truth | [`AGENTS.md`](../AGENTS.md), [`CURRENT_STATE.md`](../CURRENT_STATE.md), [`VERSION.md`](../VERSION.md), [`CHANGELOG.md`](../CHANGELOG.md) | Product version `5.0.0`, family `3.0.0`, current evidence, boundaries, and the selected movement |
...
| Plural owner-native learning | Returns each reusable learning to the closest competence it must change, then tests it in later non-identical work | [`EVOLUTION_CONTRACT.json`](../kernel/EVOLUTION_CONTRACT.json), [`COMPETENCE_CULTIVATION_PROTOCOL.md`](../kernel/COMPETENCE_CULTIVATION_PROTOCOL.md) |
...
- forms or evolves competences when intent, knowledge or emerging possibilities make them useful.
...
| `maios-project-competence-formation` | Forms or evolves the smallest useful project-local competence from intent, knowledge, successful work, emerging possibilities or a material gap |
...
-> result, new knowledge, possibility or correction
-> reusable difference changes the closest owner
-> later non-identical work uses, revises, or invalidates that learning.
```

Intent, knowledge, successful work, emerging possibilities or a material gap
can make a competence worth forming or evolving. The formation faculty gives
that capacity the smallest useful body: a method, reference, protocol, function,
skill or coordinating relation. Its methods and knowledge remain owned by the
receiving project.
...
| --- | --- | --- |
| `codex` | Codex | `.agents/skills/` with all portable owners |
...
- uninstall requires the valid current receipt and removes only unchanged
  installer-owned files, retaining existing-project directories;
...
migration: applying a different product version is not an
...
When a later consequence changes what an earlier situation means, preserve
what happened and what was understood then; reconstitute the relation from the
current evidence. The FDLA reference carries this distinction into the work.
General understanding, possibility formation, knowledge acquisition and
source-grounded expression are now reachable through dedicated living owners.

For integrity diagnostics, `python maios.py operating-status` checks the current
frontier and explicit dependencies. `python maios.py audit-continuum` also checks
cold genealogy and reports `operational_valid` separately from
`genealogical_valid`. It does not delete or repair historical receipts.
